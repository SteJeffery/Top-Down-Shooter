...
struct Animation
{
	Animation():m_Filename(NULL), m_CurrentFrame(0), m_fps(0), m_CurrentTime(0.0f), m_TimePerFrame(0.0f), m_bLoop(false),m_UIObject(false), m_bPaused(false), m_bFinished(false) {}
...
public:
	...
	void AddAnimation(int animationId, const char* filename, std::vector<sf::IntRect>& frames, std::vector<sf::Vector2f>& origins, int fps, bool loop = true, bool UIObject = false);
